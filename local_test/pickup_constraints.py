...
class CreateDistanceEvaluator(object): # pylint: disable=too-few-public-methods
    ...
    def __init__(self, data):
        ...
        if data.num_locations < 100:
            ...
            for index in xrange(data.num_locations):
                min_distance = (vincenty_distance(data.locations[0], data.locations[index]))*2
                if min_distance > data.maximum_distance:
                    self._violated_points.append(data.locations[index])
                    data.remove_location(index)
            ...
            for from_node in xrange(data.num_locations):
                ...
                for to_node in xrange(data.num_locations):
                    ...
                    if from_node == to_node or from_node == 0:
...
def add_distance_soft(routing, data, distance_evaluator):
    ...
    for vehicle_id in xrange(data.num_vehicles):
        distance_dimension.SetEndCumulVarSoftUpperBound(vehicle_id, data.maximum_distance, 100000) 
...
def add_parcels_constraints(routing, data, parcels_evaluator):
    ...
    # cost = 0 if cumul > min_parcels
    for vehicle_id in xrange(data.num_vehicles):
        parcels_dimension.SetEndCumulVarSoftLowerBound(vehicle_id, data.min_parcels, 100000) 